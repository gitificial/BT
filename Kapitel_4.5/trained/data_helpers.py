...
#-------------------- Generate TRAIN subst --------------------------------------
    positive_examples_lowerhalf_nounsSubst = list(open("./data/positive_lowerhalf_shuffled.txt").readlines())
    positive_examples_lowerhalf_nounsSubst = [s.strip() for s in positive_examples_lowerhalf_nounsSubst]

    negative_examples_lowerhalf_nounsSubst = list(open("./data/negative_lowerhalf_shuffled.txt").readlines())
...
#-------------------- Generate TEST  --------------------------------------
    # non substituted (nouns) shuffled positive examples
    positive_examples_upperHalf = list(open("./data/positive_upperhalf_shuffled.txt").readlines())
    positive_examples_upperHalf = [s.strip() for s in positive_examples_upperHalf]

    # non substituted (nouns) shuffled negative examples
    negative_examples_upperHalf = list(open("./data/negative_upperhalf_shuffled.txt").readlines())
...
def load_data():
    ...
    # sentences_train_padded = pad_sentences(sentences_train)
    # sentences_test_padded = pad_sentences(sentences_test)
    ...
    # sentences_all_padded = np.concatenate([sentences_train_padded, sentences_test_padded], 0)
    vocabulary, vocabulary_inv = build_vocab(sentences_all_padded)


    # x_train, y_train = build_input_data(sentences_train_padded, labels, vocabulary)
    # x_test, y_test = build_input_data(sentences_test_padded, labels, vocabulary)
    ...
    # inp_data = build_input_data(sentences_padded, labels, vocabulary)
    # inp_data = x, y
    """
    with open('data.pickle', 'wb') as f:
        pickle.dump(inp_data, f)
    """
